        ...
        if ($validate && $this->project_model->getIdUser($this->input->post('email')) == $_SESSION['user_id']) {
            if ($this->input->post('terms') == "1") {
                ...
                if ($query) {
                    ...
                    insertLogActivity('sign', $oi);
                } else {
                    $this->session->set_flashdata('error', 'Error: it was not possible to sign this document');
                }
            } else if ($this->input->post('terms') == "2") {
                ...
                if ($query) {
                    ...
                    insertLogActivity('unsubscribe', $oi);
                } else {
    ...
    public function login()
    {
        ...
        if ($validate) {
            $newdata = array(
                ...
                'logged_in' => TRUE,
                'photo_path' => $validate[0]->photo_path,
            );
            $this->session->set_userdata($newdata);
            $_SESSION['profile_photo_path'] = $newdata['photo_path'];
                redirect(base_url("projects"));
        } else {
